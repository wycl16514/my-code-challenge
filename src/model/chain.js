...
export class DenomUnits {
    construct(denom, exponent, aliases) {
        this.denom = denom 
...
export class ChainCoin {
    constructor(description = '', denom_uints = null, 
    base = '', name = '', display = '', symbol = '',
    logo_URIs, coingecko_id = '', ibc = null, type_asset = '',
    address = '') {
        this.description = description
        this.denom_uint = denom_uints 
        this.base = base 
        this.name = name 
        this.display = display 
        this.symbol = symbol 
        this.logo_URIs = logo_URIs
...
export class DataModel{
    ...
    get_chain_id() {
        return this.chains.chain_id
    }

    get_chain_coin(idx) {
        if (idx >= this.chains.assets.length) {
            return null 
        }

        let chain_coin = this.chains.assets[idx]

        return chain_coin
    }

    get_chain_coins(start, end) {
        if (start < 0 || start >= this.chains.assets.length) {
            return null 
        }

        if (end < 0 || end >= this.chains.assets.length) {
            return null 
        }

        return this.chains.assets.slice(start, end)
    }

    init_chain_coin(chain_coin) {
        let description = chain_coin.description?  chain_coin.description : ''
        let denom_uints = []
        for (let unit in chain_coin.denom_uints) {
            let denom_unit = new DenomUnits(unit.denom, unit.exponent, unit.aliases)
            denom_uints.push(denom_unit)
        }
        let base = chain_coin.base
        let name = chain_coin.name 
        let display = chain_coin.display 
        let symbol = chain_coin.symbol 
        let logo_URIs = {}
        logo_URIs.png = chain_coin.logo_URIs.png 
        logo_URIs.svg = chain_coin.svg? chain_coin.svg: ''
        let coingecko_id = chain_coin.coingecko_id? chain_coin.coingecko_id : ''
        let ibc = chain_coin.ibc 
        let type_asset = chain_coin.type_asset? chain_coin.type_asset : ''
        let address = chain_coin.address ? chain_coin.address : ''

        return new ChainCoin(description, denom_uints, base, name ,display,
            symbol, logo_URIs, coingecko_id, ibc, type_asset, address)
    }
}

const data_model = new DataModel(osmosis)
